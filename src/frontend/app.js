...
if (production) {
  ...
  configure_address = "http://10.144.216.186:30001";
}
...
app.get("/", function (req, res) {
  ...
  axios.all([getUsers(), getWatchers()]).then(
    axios.spread(function (users, watchers) {
      // console.log(watchers.data)
      // Convert epoch to date
      for (watcher_id in watchers.data) {
        epoch = watchers.data[watcher_id].last_run;
        watchers.data[watcher_id].last_run = new Date(
          epoch * 1000
